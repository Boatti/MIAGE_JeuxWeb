window.addEventListener('DOMContentLoaded', function(){
    ...
    /*
        // Tire une flèche Avec un modèle 3D
    function shootArrow(scene, bow, material, targetPoint, grid, currentPlayer) {
        // Crée une sphère au lieu d'un cylindre
        // const arrow = BABYLON.MeshBuilder.CreateSphere("arrow", {diameter: 0.7}, scene);
        // arrow.position = bow.position.clone();
        ...
        // Importer le modèle 3D de la flèche
        BABYLON.SceneLoader.ImportMesh("", "./models/uploads_files_4137182_Wooden+Arrow/glTF/", "scene.gltf", scene, function (newMeshes) {
            const arrow = newMeshes[0];
            arrow.position = bow.position.clone();
            arrow.scaling = new BABYLON.Vector3(20, 20, 20);
            arrow.rotation.y = Math.PI / 2; // Pivoter de 90 degrés autour de l'axe y
            arrow.material = material;
            arrow.lookAt(targetPoint);
        
            // Animation de la flèche
            const animArrow = new BABYLON.Animation("animArrow", "position", 60, BABYLON.Animation.ANIMATIONTYPE_VECTOR3, BABYLON.Animation.ANIMATIONLOOPMODE_CYCLE);
            const keysArrow = [{ frame: 0, value: arrow.position }, { frame: 100, value: targetPoint }]; // 100 frames pour l'animation
            animArrow.setKeys(keysArrow);
            arrow.animations.push(animArrow);
            scene.beginAnimation(arrow, 0, 100, false); // 100 frames pour l'animation
    ...
    */
    ...
    // Réinitialise le jeu en remettant tous les carrés à une couleur neutre
    function resetGame(grid, scene, currentPlayer) {
        ...
        // Réinitialiser le joueur actuel à 1 ou à 2, selon votre logique de jeu
        currentPlayer = 1;
        ...
        // Optionnellement, vous pouvez ajouter plus de logique ici pour une réinitialisation complète
    }
